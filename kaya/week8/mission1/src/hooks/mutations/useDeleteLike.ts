import { type RequestLpDto, type ResponseLpDto, type Likes } from './../../type/lp';
...
function useDeleteLike() {
    return useMutation({
        ...
        onMutate: async (lp: RequestLpDto) => {
            // 1. 이 게시글에 관련된 쿼리를 취소 (캐시된 데이터를 새로 불러오는 역할)
            ...
            // 복사하는 가장 큰 이유는 나중에 오류가 발생했을 때 이전 상태로 되돌리기 위해
            const newLpPost = {...previousLpPost};

            // 게시글에 저장된 좋아요 목록에서 현재 내가 눌렀던 좋아요의 위치를 찾아야 함
            const me = queryClient.getQueryData<ResponseLpDto>([
                QUERY_KEY.myInfo,
            ]);
        ...
        // onSettled는 API 요청이 끝난 후 (성공, 실패 상관x 실행)
