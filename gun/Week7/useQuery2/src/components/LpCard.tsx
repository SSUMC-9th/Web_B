...
const LpCard = ({lp}:LpCardProps) => {
    ...
    const handleImageError = (e: React.SyntheticEvent<HTMLImageElement, Event>) => {
        // 이미지 로드 실패 시, 상태를 true로 설정하여 로딩 처리를 완료하고,
        // URL을 플레이스홀더/에러 이미지로 변경합니다.
        setImageLoaded(true);
    ...
    >
        ...
        <div className="object-cover w-full h-48 relative bg-gray-200 ">
            ...
            {/* 실제 이미지 */}
            <img
                // 💡 이미지 URL 상태 사용 (실패 시 FALLBACK_IMAGE_URL)
                src={imageUrl} 
                alt={lp.title}
                className={`object-cover w-full h-48 transition-opacity duration-500 
                            ${imageLoaded ? 'opacity-100' : 'opacity-0'}`} 
                onLoad={handleImageLoad} // 로드 성공 시 opacity를 1로 전환
                onError={handleImageError} // 로드 실패 시 URL 변경 및 로드 완료 처리
            />
        </div>
